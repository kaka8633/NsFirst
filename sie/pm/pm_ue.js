...
function addJobResourcesItem(form, type) {
	...
	if (type == 'edit') {
		...
		jobResources.addField('custpage_job', 'text', '职称',
				'customlist_job_title').setDisplayType('Disabled');// 职称
		jobResources.addField('custpage_power', 'text', '能力',
...
function dataTransfer() {
	...
	var custbody_curtain_wall_drug_safety = request
			.getParameter('custbody_curtain_wall_drug_safety');// 幕墙安评
	var custbody_tender_for_q_a_review = request
			.getParameter('custbody_tender_for_q_a_review');// 招标答疑评审
	var custbody_test_station = request.getParameter('custbody_test_station');// 试验旁站
	var custbody_project_final_accounts = request
			.getParameter('custbody_project_final_accounts');// 项目决算
	var custbody_animation_design = request
			.getParameter('custbody_animation_design');// 动画设计
	var custbody_construction_e_appraisal = request
			.getParameter('custbody_construction_e_appraisal');// 建筑工程鉴定
	var custbody_in_depth_design_not_icd = request
			.getParameter('custbody_in_depth_design_not_icd');// 深化设计（不含施工图)
	var custbody_the_tender_list = request
			.getParameter('custbody_the_tender_list');// 招标清单
	var custbody_selection_of_c_materials = request
			.getParameter('custbody_selection_of_c_materials');// 施工材料资源筛选
	var custbody_site_factory_inspections = request
			.getParameter('custbody_site_factory_inspections');// 现场（工厂）检查
	var custbody_completion_a_cooperation = request
			.getParameter('custbody_completion_a_cooperation');// 竣工验收配合
	var custbody_bim_design = request.getParameter('custbody_bim_design');// BIM设计
	var custbody_program_coordination_ir = request
			.getParameter('custbody_program_coordination_ir');// 方案配合（含送审）
	var custbody_bidding_budget = request
			.getParameter('custbody_bidding_budget');// 招标预算
	var custbody_construction_unit_dr_or_o = request
			.getParameter('custbody_construction_unit_dr_or_o');// 施工单位图纸审核或优化
	var custbody_in_field_service = request
			.getParameter('custbody_in_field_service');// 驻场服务
	var custbody_a_rendering_or_effect_model = request
			.getParameter('custbody_a_rendering_or_effect_model');// 效果图或效果模型
	...
	// 服务范围
	nlapiSetFieldValue('custentity_project_coordinate', custbody_combined_with,
			true, true);// 方案配合
	nlapiSetFieldValue('custentity_mqap', custbody_curtain_wall_drug_safety,
			true, true);// 幕墙安评
	nlapiSetFieldValue('custentity_zbdyps', custbody_tender_for_q_a_review,
			true, true);// 招标答疑评审
	nlapiSetFieldValue('custentity_sypz', custbody_test_station, true, true);// 试验旁站
	nlapiSetFieldValue('custentity_xmjs', custbody_project_final_accounts,
			true, true);// 项目决算
	nlapiSetFieldValue('custentity_dhsj', custbody_animation_design, true, true);// 动画设计
	nlapiSetFieldValue('custentity_jzgcpgjd',
			custbody_construction_e_appraisal, true, true);// 建筑工程鉴定
	nlapiSetFieldValue('custentity_shsj', custbody_in_depth_design_not_icd,
			true, true);// 深化设计（不含施工图)
	nlapiSetFieldValue('custentity_zbqd', custbody_the_tender_list, true, true);// 招标清单
	nlapiSetFieldValue('custentity_sgcl', custbody_selection_of_c_materials,
			true, true);// 施工材料资源筛选
	nlapiSetFieldValue('custentity_xcjc', custbody_site_factory_inspections,
			true, true);// 现场（工厂）检查
	nlapiSetFieldValue('custentity1', custbody_completion_a_cooperation, true,
			true);// 竣工验收配合
	nlapiSetFieldValue('custentity_bmi', custbody_bim_design, true, true);// BIM设计
	nlapiSetFieldValue('custentity_faph', custbody_program_coordination_ir,
			true, true);// 方案配合（含送审）
	nlapiSetFieldValue('custentity_zbys', custbody_bidding_budget, true, true);// 招标预算
	nlapiSetFieldValue('custentity_sgdw', custbody_construction_unit_dr_or_o,
			true, true);// 施工单位图纸审核或优化
	nlapiSetFieldValue('custentity_zcfw', custbody_in_field_service, true, true);// 驻场服务
	nlapiSetFieldValue('custentity_xgt', custbody_a_rendering_or_effect_model,
			true, true);// 效果图或效果模型
...
function contractsData(form) {
	...
	if (result[0] != undefined) {
		...
		var sales100search = nlapiLoadSearch(null,
				'customsearch_contract_sales_order_100');
		sales100search.addFilter(new nlobjSearchFilter('internalid', 'jobMain',
				'is', jobId));
...
function syncContractsData(form) {
	...
	// 创建销售合同的子列表
	var contractsSublist = form.addSubList('custpage_sale_contracts_list_sync',
			'list', '销售合同(同步)', 'relrecords');
	...
	contractsSublist.addField('custpage_contracts_finished_money_sync', 'text',
			'已完工金额');
	contractsSublist.addField('custpage_contracts_unfinished_money_sync',
			'text', '未完工金额');
	contractsSublist.addField('custpage_contracts_invoice_income_sync', 'text',
			'开票收入');
	contractsSublist.addField('custpage_contracts_uninvoice_income_sync',
			'text', '未开票收入');
	contractsSublist.addField('custpage_contracts_cash_sync', 'text', '现金(回款)');
	contractsSublist.addField('custpage_contracts_receivables_sync', 'text',
			'应收账款');
	...
	// 未完成金额
	var unfinishedTotal = jobRecord
			.getFieldValue('custentity_not_finished_amount');
	// 开票金额
	var invoiceIncome = jobRecord.getFieldValue('custentity_bill_amount');
	// 未开票金额
	var uninvoiceIncomeValue = jobRecord
			.getFieldValue('custentity_not_bill_amount');
	// 现金（回款）
	...
	// 子列表赋值
	contractsSublist.setLineItemValue('custpage_contracts_codes_sync', 1,
			tranid);
	contractsSublist.setLineItemValue('custpage_contracts_name_sync', 1,
			contractName);
	contractsSublist.setLineItemValue('custpage_contracts_status_sync', 1,
			statusValue);
	contractsSublist
			.setLineItemValue('custpage_contracts_money_sync', 1, total);
	contractsSublist.setLineItemValue('custpage_contracts_finished_money_sync',
			1, finishedTotal);
	contractsSublist.setLineItemValue(
			'custpage_contracts_unfinished_money_sync', 1, unfinishedTotal);
	contractsSublist.setLineItemValue('custpage_contracts_invoice_income_sync',
			1, invoiceIncome);
	contractsSublist
			.setLineItemValue('custpage_contracts_uninvoice_income_sync', 1,
					uninvoiceIncomeValue);
	contractsSublist.setLineItemValue('custpage_contracts_cash_sync', 1, cash);
	contractsSublist.setLineItemValue('custpage_contracts_receivables_sync', 1,
			receivables);
...
function batchModifyTaskManager() {
	...
	if (director != null || manager != null) {
		...
		taskFilters[1] = new nlobjSearchFilter('custevent_milestone', null,
				'is', 'T');
		...
		if (taskResults) {
			for (var i = 0; i < taskResults.length; i++) {
				...
				var cdirector = taskResults[i]
						.getValue('custevent_project_director');
				var cmanager = taskResults[i]
						.getValue('custevent_project_manager');
				;

				if (director != cdirector) {
					nlapiSubmitField('projecttask', taskId,
							'custevent_project_director', director, true);
				}

				if (manager != cmanager) {
					nlapiSubmitField('projecttask', taskId,
							'custevent_project_manager', manager, true);
				}
			}
		}
	}
