...
function portletName(portletObj, column) {
	...
	//test
//	var data = nlapiLoadSearch(null,'customsearch_funnnel_data').runSearch().getResults(0,1000);
//	nlapiLogExecution('error','funelData',JSON.stringify(data));
	...
	newSearch.addFilter(new nlobjSearchFilter('trandate',null, 'on','2018-9-17'));
//	var searchResults = newSearch.runSearch().getResults(0,1000);
	...
	do{
		...
		if(doResult.length > 0 ){//过滤空值插入
			returnData.push(doResult);
		}
		doStart = doStart + 1001;
		doEnd = doEnd + 1001;
	}while(doResult.length > 0);
//	searchResults.forEachResult(function(searchResult){
//		returnData.push(searchResult.getValue('trandate'));
//		return true;
//	});
	nlapiLogExecution('error','returnData',JSON.stringify(returnData[0]));
	
	
	
	return;
	var loadResult = nlapiLoadSearch(null,'customsearch_funnnel_data');
	var searchResult = loadResult.runSearch();
	var data = searchResult.getResults(0,1000);
	nlapiLogExecution('error','funelData',JSON.stringify(data));
	return;
